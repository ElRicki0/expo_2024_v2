...
SAVE_FORM.addEventListener('submit', async (event) => {
    ...
    (ID_BENEFICIO.value) ? action = 'updateRow' : action = 'createRow';
    // Constante tipo objeto con los datos del formulario.
    console.log(ID_BENEFICIO.value);
...
const openDelete = async (id) => {
    ...
    const RESPONSE = await confirmAction('¿Desea eliminar el beneficio de forma permanente?');
    ...
    if (RESPONSE) {
        ...
        const DATA = await fetchData(BENEFICIO_API, 'deleteRow', FORM);
        // Se comprueba si la respuesta es satisfactoria, de lo contrario se muestra un mensaje con la excepción.
        if (DATA.status) {
            // Se muestra un mensaje de éxito.
