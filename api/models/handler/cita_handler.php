...
class Citahandler
{
    ...
    {
        $sql = 'SELECT ct.id_cita, ct.nombre_cita, ct.fecha_creacion_cita, ct.fecha_asignacion_cita, ct.estado_cita, ct.numero_seciones, c.nombre_cliente, ct.id_cliente, s.tipo_servicio
                from tb_citas ct
                join tb_clientes c on ct.id_cliente = c.id_cliente
                join tb_servicios s on ct.id_servicio = s.id_servicio';
...
';
    ...
    public function readCantidadClientePendiente()
    {
        $sql = 'SELECT c.nombre_cliente,
                ...
                FROM tb_citas ct
                JOIN tb_clientes c ON ct.id_cliente = c.id_cliente
                WHERE ct.estado_cita LIKE "pendiente"
                GROUP BY c.id_cliente
                ORDER BY cantidad_citas 
                DESC limit 5';
        return Database::getRows($sql);
    }
    ...
    {
        $sql = 'SELECT estado_cita,
                COUNT(*) AS cantidad_citas
                FROM tb_citas
                WHERE estado_cita IN ("pendiente", "proceso", "terminado")
    ...
    public function graficoBarrasPrediccionCitas()
    {
        $sql = 'WITH CitasSemanales AS (
                ...
                FROM tb_citas
                WHERE YEARWEEK(fecha_asignacion_cita, 1) = YEARWEEK(NOW(), 1) GROUP BY id_servicio)
                ...
                END, 0) AS prediccion_proxima_semana
                FROM tb_servicios s
                LEFT JOIN CitasSemanales cs ON s.id_servicio = cs.id_servicio
    ...
    public function graficoEntreFechas()
    {
        $sql = 'SELECT s.tipo_servicio, 
                ...
                WHERE c.fecha_creacion_cita BETWEEN ? AND ?
    ...
    {
        $sql = 'SELECT cl.nombre_cliente, s.tipo_servicio,
                ...
                WHERE s.id_servicio = ?
                GROUP BY cl.nombre_cliente, s.tipo_servicio
    ...
    {
        $sql = 'SELECT c.id_cita, c.nombre_cita, c.fecha_creacion_cita, c.fecha_asignacion_cita, c.estado_cita, c.numero_seciones,
                    cl.nombre_cliente, cl.apellido_cliente, s.tipo_servicio, e.nombre_empleado, e.apellido_empleado
                FROM tb_citas c
